...
const DeliveryManagement = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [ordersRes, overdueRes] = await Promise.all([
        getAdminOrders({ status: "SHIPPED", size: 50 }),
        getOverdueDeliveries(),
      ]);
  ...
  const handleCreateTracking = async (orderId) => {
    try {
      ...
      setTrackingForm({ trackingNumber: "", courierName: "", estimatedDeliveryDate: "" });
      fetchData();
    } catch (err) {
  ...
  return (
    <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                {Math.max(0, orders.length - overdueOrders.length)}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {(activeTab === "all" ? orders : overdueOrders).length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-500">
              {activeTab === "overdue" ? "All deliveries are on schedule!" : "Orders that need shipping will appear here"}
            </p>
          </div>
        ) : (
          <div className="divide-y divide-gray-200">
            {(activeTab === "all" ? orders : overdueOrders).map((item) => (
              <div key={item.id || item.orderNumber} className="p-6 hover:bg-gray-50">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    <DeliveryStatusBadge status={item.status || item.deliveryStatus || "PREPARING"} />

                    {/* Update Status Dropdown */}
                    <select
                      onChange={(e) => {
                        if (e.target.value) {
                          handleUpdateStatus(item.id || item.orderId, e.target.value);
                          e.target.value = "";
                        }
                      }}
                ...
                {/* Tracking Form (for orders without tracking) */}
                {!item.trackingNumber && selectedOrder === item.id && (
